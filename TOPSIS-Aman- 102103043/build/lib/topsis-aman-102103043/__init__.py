...
def validate_inputs(weights, impacts, num_columns):
    ...
    if len(weights) != num_columns - 1 or len(impacts) != num_columns - 1:
        print("Error: Number of weights, impacts, and columns (excluding the first column) must be the same.")
        sys.exit(1)
...
def main():
    if len(sys.argv) != 5:
        print("Usage: python topsis_script.py input_file.csv weights impacts output_file.csv")
        sys.exit(1)
    ...
    input_df = read_csv('topsis-aman-102103043/102103043-data.csv')
    ...
    try:
        ...
        write_csv('topsis-aman-102103043/102103043-result.csv', 'topsis-aman-102103043/102103043-data.csv')
